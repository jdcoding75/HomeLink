...
struct CatchBadgeView: View {
    ...
    /// [copy-declutter] collapsed to a single line — old 5-tier ladder preserved below.
    private var label: String {
        "New thoughts in your bucket"
        // was:
        // switch count {
    ...
    var body: some View {
        // [copy-declutter] never render at zero (call site also guards queueCount > 0)
        if count > 0 {
            Button(action: onTap) {
                HStack(spacing: 6) {
                    Text(label)
                        .font(.system(size: 13, design: .serif).italic())
                    // [copy-declutter] trailing decorative ✦ dropped — bare label.
                    // Text("✦")
                    //     .font(.system(size: 12))
                }
