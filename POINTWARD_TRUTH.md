...
The connection **MECHANISM works** (forms / persists / displays — proven this session). The
**CONTACT-LIFECYCLE layer** on top needs completion. These **INTERLOCK** (same
`stampConnections` / `syncConnections` / `link_connections` machinery) — treat as **ONE system to
complete**, not separate bugs:
- **CONTACT RECONCILIATION.** A manual contact (no `senderID`) for the same person as a connected contact
  can't be matched → duplicates + orphans + inconsistent routing (confirmed live: **"Bob" stayed
  unconnected while "Joshua" = same person** connected). **FIX — RULE 1: ONE CONTACT PER USER ID** — on an
  id-collision, **WARN** *"[Bob] is the same person as [Joshua]; one contact per person — which to keep?"*,
  user picks, the other is merged/discarded.
- **DELETE-DISCONNECT.** Deleting a paired contact does NOT remove the server `link_connections` row → it
  **RE-SURFACES on next sync** (today's persistence fallback); deleted users come back. **FIX — RULE 2:
  delete clears the server id** — needs a **NEW `delete_connection` / disconnect RPC** (none exists;
  `record_connection` only writes). **DECISION OPEN:** one-directional (sender's row) vs **BILATERAL**
  (reciprocal row too — needs SECURITY DEFINER). **Rules 1+2 together solve most cases** (duplicates,
  wrong-attach, orphans, inconsistent routing, resurface-after-delete, most test tangle).
- **SYNC-LAG (old backlog #4).** No realtime/polling for `link_connections`; the sender syncs only on
  `onAppear` + foreground → a written connection isn't seen until relaunch/foreground. _(Old #3
  sender-reinstall re-stamp = **FIXED** by `256e854`.)_
- **➡ This cluster, done + two-phone-tested, = the RELEASE GATE. When solid, v1 is shippable on connection
  grounds.**
