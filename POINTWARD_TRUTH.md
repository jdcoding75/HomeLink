...
- **Birthday + Firework — now first-class peer animations** (Special Moments Stages 1+2,
  `c38ca9d`): selectable `Instrument`+`SenderStyle` cards; sender-side dispatch keys on
  style/selection (not the emoji). **Stage 3 (recipient re-key) is still open** — until
  then Birthday's default emoji is the interim **🎂** (flip back to 🎁 at Stage 3, per
  the AnimationManifest comment + `9ab3f6e`).
...
named `reports/`. (Restore ladder runs top→bottom; HEAD at hand-off = the interim below.)
...
- **`9ab3f6e`** — interim: Birthday `defaultEmoji` 🎁 → **🎂** so the unchanged emoji-keyed
  recipient routes it correctly. **Flip back to 🎁 at Stage 3.** _(reports/birthday_default_interim.md)_
...
- **History tab** — full feature project, not just a view. Net-new data layer: **sent
  ...
  `.tag(3)`). _(reports/history_tab_audit.md)_
- **Special Moments Stage 3** — re-key `ReceiptView` from emoji → `style == .birthday/.firework`
  (closes the birthday-as-bucket gap, flips the default back to 🎁). **SQL gate cleared**
  (`sender_style` is free text — no migration); remaining gate = read the
  `send-ping-notification` Edge Function. Likely pre-launch.
