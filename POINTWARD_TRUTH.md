...
1. **[NEXT] WEBSITE (`pointward-website`): clipboard-write** — on the "Get Pointward" tap, silently write
...
2. **APP (HomeLink): clipboard-read** — on first launch, `UIPasteboard.detectPatterns` (silent presence
...
3. **APP: unlock-everything / disable-paywall** (the Build-8 change — v1 ships 100% free).
...
- **5T. [DO AFTER BUILD #1/A] PASS 2 — CLOSED-APP PUSH** — the **ORIGINAL root-cause gate, NEVER RUN
...
7. **Sender not notified of connection after a cold recipient received** (two-phone test). **NOT the
...
9. **Auto-create / connected-stamp on receive (backlog #3/#4):** a sender does NOT auto-appear on the
...
### PARKED (post-launch)
Compass v2 (red marker + haptic + lock-on-target, after a Design exploration); true live per-thought web
animation; App Clips (only if cold-conversion data demands fully-automatic no-tap delivery); health-audit
cleanup (Tier 1: delete `ProSetupView` ~1019 dead lines, etc.); animation/app code separation;
