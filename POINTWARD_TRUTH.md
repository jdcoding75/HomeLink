...
**E · MONETIZATION — v1 SHIPS FULLY OPEN (paywall deferred).** **DECISION (SUPERSEDES the earlier
same-day "free = compass+emojis, locked = rest" framing):** v1 ships with **NO paywall — ALL
animations/instruments FREE**; a paywall may come **later, only if traction.** Near-term: a small
**"unlock everything for v1"** app change (disable the live `PaywallView` gating — it's LIVE in
Release, only `#if DEBUG`-suppressed in dev) **AFTER** the receive/push test. Optional fast-follow:
a voluntary **"buy the dev a coffee"** support option (**NOT** a gate) with a **FREE-FOREVER-FOR-
SUPPORTERS** promise → **supporter status MUST be RECORDED when accepted** so a future paywall can
honor it. **Apple-IAP caveat:** digital in-app support generally needs Apple IAP, not an external
link — confirm at build time. **SUPERSEDES the tester-unlock** (everything open → no special unlock;
...
### ⭐ LAUNCH / MONETIZATION STRATEGY — v1 = FULLY OPEN (paywall deferred)
**⭐ v1 DECISION — SHIP FULLY OPEN (2026-06-22, SUPERSEDES the earlier same-day "free = compass+emojis
/ locked = rest" framing):** **v1 ships with NO paywall — ALL animations/instruments are FREE.** A
paywall may be added **LATER, and only if the app gains traction.**
- **NEAR-TERM APP WORK (do AFTER the current receive/push two-phone test — NOT a launch blocker
  beyond this):** a small **"unlock everything for v1"** change — remove/disable the live `PaywallView`
  gating so every instrument is free in Release. (`PaywallView` is currently **LIVE in Release** and
  was only `#if DEBUG`-suppressed in dev — this change makes Release match "fully open.")
- **COFFEE-SUPPORT (OPTIONAL — v1 or fast-follow; NEW; NOT a gate):** a voluntary **"buy the solo
  developer a coffee / help with costs"** option. Purely optional — it unlocks nothing extra
  (everything is already free). Not a launch blocker.
  - **⭐ PROMISE — FREE FOREVER FOR SUPPORTERS:** anyone who supports gets **everything free forever**
    if a paywall is ever later introduced. **→ Supporter status MUST be RECORDED at the moment support
    is accepted** (a durable per-user record — e.g. a server `is_supporter` / `supported_at` field) so
    a future paywall can honor it. **Do NOT ship the support option without recording who supported.**
  - **⚠️ APPLE-IAP CAVEAT:** in-app **digital** support on iOS generally **requires Apple IAP**, not an
    external donation link — **confirm the current App Store rules at build time** before picking the
    mechanism.
- **SUPERSEDES the tester-unlock:** with everything open, internal testers exercise ALL animations
  with no special unlock → the tester-unlock is **no longer the near-term path**
  (`reports/tester_unlock_spec.md` is kept but **marked SUPERSEDED-FOR-V1**, in case a paywall returns).

_If/when a paywall DOES return (only on traction), the generosity/propagation thinking below is the
starting point — and the coffee-supporters are grandfathered free per the promise above._
