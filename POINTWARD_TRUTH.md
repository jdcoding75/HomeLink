...
## § FUTURE RELEASES — Product Roadmap
_Reconstructed 2026-06-26 from session decisions — may be incomplete; reconcile if
original notes surface. Items already detailed above (§K, catch-bucket, walking-send,
location sharing, directional catch, §C device check, arrival order) are cross-referenced
here for a single roadmap view._

### Next Release (post-launch v2)
- Opt-in per-person location sharing (one-time snapshot at send) — see location bank above
- Directional compass catch mechanic — Lab prototype `15aca02`, promote when proven
- §K wand dedicated receipt routing — audited, parked, ready when TestFlight data informs
- Walking-accidental-send — resolve after TestFlight week (B1 tap-to-send if frequent)
...
- Interactive catch receipts across all instruments (Lab → live when proven)
- Two-phone live compass (real-time, significant new plumbing)
- Bucket organization (folders, favorites, pinning)
- Read receipts / scheduled sends / group sends
- Subscription/Pro tier (build AFTER TestFlight data shows what people value)
- Settings second-pass (privacy, Help/FAQ, notification preferences)

### Technical / Evergreen
- Animation Lab → live promotions pipeline (Lab is the pipeline, not a graveyard)
- Code cleanup + CompassView monolith refactor (AFTER v1 stable, not before)
- Xcode/Swift version migrations (annual, 6 months post major release)
- Legacy anon key → modern publishable key migration (standalone, before any v2 release)
- ComposeBackView ↔ OnboardingView copy/showcase polish
- Emoji picker / CreateThoughtSheet (dormant, revisit when personalization is priority)
